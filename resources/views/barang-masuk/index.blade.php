...
<div class="space-y-6 animate-fade-in">
    ...
    <div class="flex flex-col gap-4 print:hidden">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <p class="text-sm text-slate-500">Mencatat penambahan stok sparepart yang masuk dari supplier atau restok bengkel.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <!-- Unduh PDF -->
                <a href="{{ route('barang-masuk.index', array_merge(request()->query(), ['export' => 'pdf'])) }}" 
        ...
        <div class="bg-white border border-slate-150 rounded-2xl shadow-sm p-4 md:p-5">
            <form action="{{ route('barang-masuk.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                <div>
    ...
    <div class="hidden print:block text-center mb-6">
        <h1 class="text-2xl font-bold text-slate-950">Laporan Barang Masuk</h1>
        ...
        <div class="overflow-x-auto">
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead>
                        <tr class="bg-slate-50 text-slate-500 text-xs font-bold uppercase tracking-wider print:bg-slate-100">
                            ...
                            <th scope="col" class="px-6 py-4 text-center">Jumlah Masuk</th>
                            <th scope="col" class="px-6 py-4 text-right">Harga Beli</th>
                            <th scope="col" class="px-6 py-4 text-right">Harga Total</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @foreach($incomingLogs as $log)
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-bold text-emerald-600 bg-emerald-50/50 rounded-lg">
                                    +{{ $log->jumlah }} {{ $log->sparepart->satuan }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-slate-600 tabular-nums">
                                    @if($log->harga_beli > 0)
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right tabular-nums">
                                    @if($log->harga_total > 0)
                                        <span class="text-sm font-bold text-blue-700 bg-blue-50 px-2.5 py-1 rounded-lg">
            ...
            @else
                <div class="flex flex-col items-center justify-center py-16 px-4 text-center">
                    ...
                    <p class="text-sm text-slate-500 max-w-xs mt-1">Tidak ditemukan transaksi barang masuk pada kriteria tersebut.</p>
                    <a href="{{ route('barang-masuk.create') }}" class="mt-4 inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-xl transition-colors">
